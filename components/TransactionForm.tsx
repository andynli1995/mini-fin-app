  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.error || `Failed to ${transaction ? 'update' : 'create'} transaction`)
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-4 sm:top-20 mx-auto p-4 sm:p-5 border w-full max-w-md sm:w-96 shadow-lg rounded-md bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700 m-4 sm:m-auto">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">{transaction ? 'Edit Transaction' : 'Add Transaction'}</h3>
          <button
            onClick={() => {
              setIsOpen(false)
              if (onClose) onClose()
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex space-x-3">
            ...
            >
              {isSubmitting ? (transaction ? 'Updating...' : 'Saving...') : (transaction ? 'Update' : 'Save')}
            </button>
            <button
              type="button"
              onClick={() => {
